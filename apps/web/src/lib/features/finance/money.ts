...
export function getDominantMoneyAmount(
  entries: Array<{
    amount: number | string | null | undefined;
    currency: string | null | undefined;
    amountInPrimaryCurrency?: number | string | null;
  }>,
) {
  ...
  const dominant = [...totals.entries()].sort(
    ([leftCurrency, left], [rightCurrency, right]) =>
      Math.abs(right.primaryAmount) - Math.abs(left.primaryAmount) ||
      leftCurrency.localeCompare(rightCurrency),
  )[0];
  return dominant
    ? {
        currency: dominant[0],
        amount: dominant[1].amount,
        amountInPrimary: dominant[1].primaryAmount,
      }
    : null;
}
