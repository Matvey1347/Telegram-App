...
describe("telegram-calendar-scheduler", () => {
  ...
  it("keeps the calendar candidate list separate from link-readiness checks", () => {
    const posts: TelegramManagedPost[] = [
      makePost({
        id: "blocked-draft",
        status: "DRAFT",
  ...
  it("shuffles every candidate instead of rotating the original order", () => {
    expect(
      shuffleCalendarSchedulablePosts(
        ["first", "second", "third", "fourth"],
        () => 0,
      ),
    ).toEqual(["second", "third", "fourth", "first"]);
  });
  ...
  it("sorts reversed batch assignments chronologically before scheduling", () => {
    expect(
      sortScheduleManagedPostAssignments([
        { postId: "post-b", scheduledAt: "2026-07-29T12:15:00.000Z" },
        { postId: "post-a", scheduledAt: "2026-07-29T08:15:00.000Z" },
