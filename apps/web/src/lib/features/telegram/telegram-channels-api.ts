...
const telegramChannelsApi = {
  ...
  attachCustomEmojiPack: async (channelId: string, packId: string, target: TelegramCustomEmojiPackTarget) =>
    (
      await api.post<TelegramChannelCustomEmojiPacksResponse>(
        `/telegram-channels/${channelId}/custom-emoji-packs/${packId}/attach`,
        { scope: target.scope, ...(target.scope === "CHANNELS" ? { channelIds: target.channelIds } : {}) },
      )
    ).data,
  detachCustomEmojiPack: async (channelId: string, packId: string, target: TelegramCustomEmojiPackTarget) =>
    (
      await api.delete<TelegramChannelCustomEmojiPacksResponse>(
        `/telegram-channels/${channelId}/custom-emoji-packs/${packId}`,
        { data: { scope: target.scope, ...(target.scope === "CHANNELS" ? { channelIds: target.channelIds } : {}) } },
      )
    ).data,
  ...
  gptContext: async (id: string) =>
    (await api.get<Blob>(`/telegram-channels/${id}/gpt-context`, { responseType: "blob" })).data,
  ...
  verifyManagedPostTelegramId: async (channelId: string, postId: string) => (await api.post<TelegramManagedPost>(`/telegram-channels/${channelId}/managed-posts/${postId}/verify-telegram-id`)).data,
  verifyManagedPostTelegramIds: async (channelId: string) => (await api.post<import("@telegram-system/shared").TelegramManagedPostVerificationResult>(`/telegram-channels/${channelId}/managed-posts/verify-telegram-ids`)).data,
