...
import { Button, PageHeader, Table } from "@/components/ui/primitives";
  ...
  return (
    <AppShell>
      ...
      <QueryContentState
        ...
        errorText="Failed to load scheduled tasks."
        emptyText="No scheduled tasks are available for this workspace."
      >
        <>
...
function TaskSection({
  ...
  const groups = new Map<string, ScheduledTaskView[]>();
  for (const task of tasks) {
    const key = task.group?.key ?? "OTHER";
...
function TaskGroup({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900/70">
      {group ? (
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-neutral-800 bg-neutral-950/70 px-3 py-3">
          <div>
            <h4 className="font-medium text-white">{group.name}</h4>
            <p className="mt-1 text-xs text-neutral-400">{group.description}</p>
          </div>
          {tasks[0]?.workspaceId && groupKey === "TELEGRAM" ? (
