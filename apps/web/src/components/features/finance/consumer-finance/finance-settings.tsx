...
function tierLabel(
  ...
  t: ReturnType<typeof financeCopy>,
) {
  return tier === "PRO"
    ? t.financePro
    : tier === "ULTIMATE"
...
export function FinanceSettings({
  ...
  const billing = useQuery({
    ...
    enabled: entitlements.data?.tier !== undefined && entitlements.data.tier !== "ULTIMATE",
  });
  ...
  const refreshBilling = () => Promise.all([
    client.invalidateQueries({ queryKey: consumerFinanceKeys.entitlements(botId) }),
    client.invalidateQueries({ queryKey: consumerFinanceKeys.billing(botId) }),
  ]);
          ...
          <div className="mt-2 space-y-3">
            ...
            {entitlement.tier !== "ULTIMATE" ? (
              billing.isLoading ? (
                ...
                <div className="space-y-2">
                  <ErrorState text={t.planLoadError} />
                  <Button variant="secondary" onClick={() => billing.refetch()}>
                    {t.retry}
                  </Button>
                </div>
              )
            ...
            {entitlement.tier !== "FREE" ? (
              <div className="flex flex-wrap gap-2">
                <Button variant="secondary" disabled={paymentPortal.isPending} onClick={() => paymentPortal.mutate()}>
                  {t.managePaymentDetails}
                </Button>
                <Button variant="secondary" disabled={cancelAutoRenew.isPending || resumeAutoRenew.isPending} onClick={() => entitlement.cancelAtPeriodEnd ? resumeAutoRenew.mutate() : cancelAutoRenew.mutate()}>
                  {entitlement.cancelAtPeriodEnd ? t.resumeAutoRenew : t.cancelAutoRenew}
                </Button>
              </div>
            ) : null}
          </div>
