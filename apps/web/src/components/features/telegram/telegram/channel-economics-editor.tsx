...
function displayName(product: Pick<TelegramAdProduct, "name" | "isPermanent">) {
  return product.isPermanent && product.name.trim() === "1/permanent"
    ? "No auto-delete"
    : product.name;
}
...
export function ChannelEconomicsEditor({
  ...
  const [localTargetCpa, setLocalTargetCpa] = useState(
    channel.targetCpa == null ? "" : String(channel.targetCpa),
  );
  const [localNormalCpa, setLocalNormalCpa] = useState(
  ...
  const setTargetCpa = (value: string) =>
    onDraftChange
      ? onDraftChange({ targetCpa: value })
      : setLocalTargetCpa(value);
  const setNormalCpa = (value: string) =>
    onDraftChange
      ? onDraftChange({ stopCpaFrom: value })
      : setLocalNormalCpa(value);
  ...
  const formatMutation = useMutation({
    mutationFn: async (
      operation:
        | { kind: "save"; draft: FormatDraft }
        | { kind: "toggle"; product: TelegramAdProduct }
        | { kind: "delete"; product: TelegramAdProduct },
    ) => {
      if (operation.kind === "delete")
        return telegramAdSalesApi.deactivateProduct(operation.product.id);
      if (operation.kind === "toggle")
        return telegramAdSalesApi.updateProduct(operation.product.id, {
          isActive: !operation.product.isActive,
        });
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: telegramAdSalesKeys.channelProducts(channel.id),
        }),
      ]);
      setEditing(null);
      setDeleting(null);
  ...
  const invalidKpi =
    targetCpa.trim() &&
    normalCpa.trim() &&
    Number(targetCpa) >= Number(normalCpa);
  const invalidFormat =
    editing &&
    (!editing.name.trim() ||
      Number(editing.topHours) <= 0 ||
      (!editing.permanent && Number(editing.feedHours) <= 0));
    ...
    <div className="space-y-5">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <FormField label="Normal — CPA up to">
            <Input
              inputMode="decimal"
              value={normalCpa}
              onChange={(event) => setNormalCpa(event.target.value)}
              placeholder="Optional"
            />
          </FormField>
        </div>
      ...
      <section className="rounded-md border border-neutral-800 bg-neutral-950/50 p-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-neutral-400">
              Used when selling ads for this channel.
            </p>
          </div>
          <Button
            variant="secondary"
            className="inline-flex items-center gap-2"
            onClick={() => setEditing(formatDraft(undefined, products.length))}
          >
        ...
        {setupQuery.isLoading ? (
          <div
            ...
            aria-label="Loading placement formats"
          >
            ...
            {products.map((product) => (
              ...
              >
                ...
                <button
                  ...
                  onClick={() =>
                    formatMutation.mutate({ kind: "toggle", product })
                  }
                  aria-label={`${product.isActive ? "Disable" : "Enable"} ${displayName(product)}`}
                  className={`rounded-full px-2 py-1 text-xs ${product.isActive ? "bg-emerald-500/15 text-emerald-300" : "bg-neutral-800 text-neutral-400"}`}
                >
                  {product.isActive ? "Active" : "Disabled"}
                </button>
                <button
                  type="button"
                  onClick={() => setEditing(formatDraft(product))}
                  aria-label={`Edit ${displayName(product)}`}
                ...
                {!standardFormats.has(product.name.trim()) ? (
                  <button
                    ...
                    aria-label={`Delete ${displayName(product)}`}
                    className="inline-flex h-8 w-8 items-center justify-center rounded-md text-neutral-400 hover:bg-red-950/50 hover:text-red-300"
                  >
                    <Trash2 size={15} />
                  </button>
                ) : null}
          ...
          <p className="mt-3 rounded-md border border-dashed border-neutral-800 p-3 text-sm text-neutral-500">
            No formats yet. Create one to make the channel bookable.
          </p>
        )}
        ...
        {editing ? (
          <div className="mt-3 space-y-3 rounded-md border border-blue-900/60 bg-blue-950/10 p-3">
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <FormField label="Hours first">
                <Input
                  inputMode="numeric"
                  value={editing.topHours}
                  onChange={(event) =>
                    setEditing({ ...editing, topHours: event.target.value })
                  }
                />
              </FormField>
              <FormField label="Hours in feed">
                <Input
                  inputMode="numeric"
                  disabled={editing.permanent}
                  value={editing.feedHours}
                  onChange={(event) =>
                    setEditing({ ...editing, feedHours: event.target.value })
                  }
                />
              </FormField>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              <ToggleRow
                ...
                onChange={(permanent) => setEditing({ ...editing, permanent })}
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                disabled={formatMutation.isPending || Boolean(invalidFormat)}
                onClick={() =>
                  formatMutation.mutate({ kind: "save", draft: editing })
                }
              >
                Save format
              </Button>
      ...
      {!embedded ? (
        <div className="flex justify-end gap-2">
          ...
          <Button
            onClick={() => economicsMutation.mutate()}
            disabled={
              economicsMutation.isPending ||
  ...
  return (
    <>
      ...
      <ConfirmDeleteModal
        ...
        onConfirm={() =>
          deleting
            ? formatMutation.mutateAsync({ kind: "delete", product: deleting })
            : undefined
        }
      />
    </>
