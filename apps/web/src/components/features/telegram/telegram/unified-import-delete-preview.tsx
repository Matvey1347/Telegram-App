...
export function UnifiedImportDeletePreview({
  ...
  onRemove?: (item: TelegramUnifiedImportPreviewItem) => void;
}) {
  const { t } = useI18n();
  ...
  const visibleItems =
    operation === "UNSCHEDULE"
      ? items
      : items.filter(
          (item) => Boolean(item.imported) === (state === "imported"),
        );
  return (
    <div>
      {operation === "DELETE" ? (
        <UnifiedImportStateTabs
          value={state}
          pendingCount={items.filter((item) => !item.imported).length}
          importedCount={items.filter((item) => item.imported).length}
          onChange={setState}
        />
      ) : null}
      <div className="grid gap-3 lg:grid-cols-2">
        {visibleItems.map((item) => (
          <article
            key={item.ref}
            className="overflow-hidden rounded-lg border border-rose-900/60 bg-rose-950/10"
          >
            <div className="flex items-center gap-2 border-b border-rose-950 px-3 py-2">
              <ActionBadge action={operation} />
              <ImportStateBadge imported={item.imported} />
              {item.icon ? <span className="text-lg">{item.icon}</span> : null}
              <strong className="min-w-0 flex-1 truncate text-sm text-white">
                {item.label}
              </strong>
            ...
            {section === "posts" ? (
              ...
              <div className="space-y-2 p-3">
                ...
                {item.status ? (
                  <span className="inline-flex rounded-full border border-neutral-700 px-2 py-0.5 text-xs text-neutral-300">
                    {item.status}
                  </span>
                ) : null}
              </div>
            ...
            {item.errors.length ? (
              <p className="border-t border-rose-900/50 px-3 py-2 text-xs text-rose-300">
                {item.errors.join("; ")}
              </p>
            ) : null}
            {failures.find((failure) => failure.ref === item.ref)?.error ? (
              <p className="border-t border-rose-900/50 px-3 py-2 text-xs text-rose-300">
                {failures.find((failure) => failure.ref === item.ref)!.error}
              </p>
            ) : null}
