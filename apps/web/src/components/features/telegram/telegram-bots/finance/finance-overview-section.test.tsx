...
describe("FinanceOverviewSection AI cost", () => {
  it("renders model and user cost for the logical bot", async () => {
    ...
    render(<QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}><FinanceOverviewSection botId="bot-1" /></QueryClientProvider>);
    expect(await screen.findByText("AI usage · current month")).toBeInTheDocument();
    expect(screen.getByText("gpt-5-mini")).toBeInTheDocument();
    expect(screen.getByText("@alice")).toBeInTheDocument();
    expect(overview).toHaveBeenCalledWith("bot-1");
  });
});
