    ...
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="rounded-lg border border-neutral-700 bg-neutral-950 p-2 text-sky-300">
            <Cpu size={20} />
          </div>
          <div>
            ...
            <p className="text-xs text-neutral-500">
              Actual provider usage for this bot and selected runtime
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-4">
        {[
          ...
          ["Cached input", data.cachedInputTokens.toLocaleString()],
          ...
          >
            ...
            <p className="mt-1 font-medium tabular-nums text-white">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <UsageTable
          ...
          rows={data.byModel.map((row) => ({
            key: row.model,
            label: row.model,
            detail: `${row.requests} requests · ${(row.inputTokens + row.outputTokens).toLocaleString()} tokens`,
        ...
        <UsageTable
          ...
          rows={data.byUser.map((row) => ({
            key: row.telegramBotUserId,
            label: row.username
              ? `@${row.username}`
              : row.firstName || row.telegramUserId,
            detail: `${row.requests} requests · Telegram ${row.telegramUserId}`,
            cost: row.estimatedCostMicros,
          }))}
        />
      </div>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-white">{row.label}</p>
                <p className="truncate text-xs text-neutral-500">
                  {row.detail}
                </p>
              </div>
              <span className="shrink-0 tabular-nums text-neutral-200">
                {formatAiCost(row.cost)}
              </span>
            </div>
          ))}
        </div>
      ) : (
        <p className="mt-2 text-sm text-neutral-500">No AI requests yet.</p>
      )}
    </div>
  );
}
