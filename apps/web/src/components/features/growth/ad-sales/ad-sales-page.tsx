...
export function AdSalesPage() {
  ...
  const channelSetupQuery = useQuery({
    queryKey: telegramAdSalesKeys.channelSetup(activeSettingsChannelId),
    queryFn: () => telegramAdSalesApi.getChannelSetup(activeSettingsChannelId),
    enabled: tab === "settings" && Boolean(activeSettingsChannelId),
    staleTime: 60 * 1000,
  });
          ...
          onCreateProduct={async (channelId, payload) => {
            ...
            await Promise.all([
              queryClient.invalidateQueries({
                queryKey: telegramAdSalesKeys.channelSetup(channelId),
              }),
              queryClient.invalidateQueries({
                queryKey: telegramAdSalesKeys.channelProducts(channelId),
              }),
...
function SettingsTab(props: {
  ...
  const policyLoading = setupLoading;
  const productLoading = setupLoading;
