...
function AccountModal({ open, onClose, onSubmit, title, initial, currencies }: { open: boolean; onClose: () => void; onSubmit: (v: Values) => void; title: string; initial?: Account; currencies: string[] }) {
  ...
  return <Modal open={open} onClose={onClose} title={title}><form className="space-y-3" onSubmit={handleSubmit(onSubmit)}><IconPicker iconId={watch('iconId') ?? null} icon={initial?.iconPresentation} onChange={(iconId) => setValue('iconId', iconId, { shouldDirty: true, shouldValidate: true })} buttonLabel="Add icon" /><FormField label="Name" required error={errors.name ? 'Required field' : undefined}><Input {...register('name', { required: true })} /></FormField><FormField label="Member"><MemberSelect value={watch('assignedMemberId')} onChange={(assignedMemberId) => setValue('assignedMemberId', assignedMemberId || null)} defaultToCurrent={!initial} /></FormField><FormField label="Currency"><CurrencySelect value={watch('currency')} currencies={currencyOptions} onChange={(currency) => setValue('currency', currency, { shouldDirty: true, shouldValidate: true })} /></FormField><FormField label="Initial Balance"><Input type="number" step="0.01" {...register('initialBalance', { valueAsNumber: true })} /></FormField><div className="flex justify-end gap-2"><Button variant="secondary" type="button" onClick={onClose}>Cancel</Button><Button type="submit">Save</Button></div></form></Modal>;
}
