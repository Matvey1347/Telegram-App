...
export default async function TelegramPostGroupsLayout({ children }: { children: React.ReactNode }) {
  const locale = normalizeAppLocale((await cookies()).get(APP_LOCALE_COOKIE)?.value);
  const catalog = (await loadCatalog(locale, "telegram/posts/groups")).default;
  return (
    <I18nPreloadedBoundary initialLocale={locale} catalogs={{ "telegram/posts/groups": catalog }}>
      {children}
