...
import { TelegramChannelAdSalesSection } from "@/components/features/growth/ad-sales/telegram-channel-ad-sales-section";
...
export default function TelegramChannelAnalyticsPage() {
  ...
  const { data: rates } = useQuery({
    queryKey: currencyKeys.latestRates(),
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="KPI settings">
        <div className="space-y-4">
          <div className="space-y-3">
            ...
            <KpiRangeFields
              label="Good up to"
              fromValue={settings.targetCpa}
              onFromChange={(value) => setValue("targetCpa", value)}
              openEnded
              upperBound
            />
            <KpiRangeFields
        ...
        {openEnded ? (
          <div className="flex items-end">
            <p className="rounded-md border border-rose-900/60 bg-rose-950/20 px-3 py-2 text-sm text-rose-200">
              {upperBound ? "This value and lower" : "This value and higher"}
            </p>
          </div>
        ) : (
          <FormField label="To ($)">
