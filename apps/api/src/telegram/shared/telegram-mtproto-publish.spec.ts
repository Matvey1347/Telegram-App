...
describe('TelegramMtprotoClient publishPost', () => {
  ...
  it('keeps t.me invite links in multi-image album captions', async () => {
    const client = new TelegramMtprotoClient();
    const fakeTelegramClient = {
      getEntity: jest.fn().mockResolvedValue('entity'),
    ...
    await client.publishPost({
      ...
      session: 'session',
      channelRef: '@bizpatterns',
      html: '<b>Album text</b>\n<a href="https://t.me/+mbliuCdzB4k0ODdk">Бізнес-патерни</a>',
      captionHtml:
        '<b>Album text</b>\n<a href="https://t.me/+mbliuCdzB4k0ODdk">Бізнес-патерни</a>',
    ...
    expect(fakeTelegramClient.sendFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        caption: [
          '<b>Album text</b>\n<a href="https://t.me/joinchat/mbliuCdzB4k0ODdk">Бізнес-патерни</a>',
        ],
        parseMode: 'html',
      }),
    );
