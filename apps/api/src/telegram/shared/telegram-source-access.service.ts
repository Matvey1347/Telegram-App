...
export class TelegramSourceAccessService {
  ...
  async recordDataSource(input: AttributionInput) {
    // This is a current source-status signal, not an unbounded success log.
    // Keep an audit row only when the observable status changes; errors are
    // always retained because they are operationally meaningful.
    const status = input.status || TelegramDataSourceStatus.SUCCESS;
    const metadata = input.metadata as Prisma.InputJsonValue;
    const previous = await this.prisma.telegramChannelDataSource.findFirst({
      where: {
        workspaceId: input.workspaceId,
        channelId: input.channelId,
        sourceId: input.sourceId,
  ...
  async publishingCapabilitiesForChannels(
    ...
    for (const channelId of uniqueChannelIds) {
      const canPublishInlineButtons = rows.some(
        (row) =>
