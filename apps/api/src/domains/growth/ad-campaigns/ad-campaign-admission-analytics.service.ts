...
export class AdCampaignAdmissionAnalyticsService {
  ...
  private selectAttributionWindowBatches<
    T extends { joinedBefore: number; releasedSubscribersCount: number },
  >(windowBatches: T[]) {
    const primaryWindowBatches = windowBatches.filter(
      (item) => item.joinedBefore === 0 && item.releasedSubscribersCount > 0,
    );
    return primaryWindowBatches.length ? primaryWindowBatches : windowBatches;
  }
