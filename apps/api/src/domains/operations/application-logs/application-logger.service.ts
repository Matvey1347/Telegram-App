...
import type { ApplicationLogKind, ApplicationLogLevel } from '@prisma/client';
...
function normalizeMinimumLevel(
  ...
  fallback: ApplicationLogLevel = 'info',
): ApplicationLogLevel {
...
function consoleLogLevels(): LogLevel[] {
  ...
  return levels
    .filter(
      ([level]) => LOG_LEVEL_PRIORITY[level] >= LOG_LEVEL_PRIORITY[minimum],
    )
    .map(([, level]) => level);
}
...
{
  private readonly minimumLevel = applicationLogMinimumLevel();
  ...
  constructor(
    ...
    super(APPLICATION_LOG_SERVICE, {
      logLevels: [...consoleLogLevels(), 'fatal'],
    });
  }
  ...
  private writeFromNest(
    ...
    const source =
      typeof contextOrTrace === 'string' && typeof maybeTrace !== 'string'
        ? contextOrTrace
