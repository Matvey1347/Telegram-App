...
import type { FinanceCapability } from '../billing/finance-entitlement.service';
import {
  FinanceUltimateQuestionDto,
  FinanceUltimateQueryDto,
...
export class FinanceUltimateController {
  ...
  @Get('overview') overview(
    ...
    return this.withUltimate(
      ...
      request,
      'FINANCIAL_FORECAST',
      (profileId) => this.ultimate.overview(profileId),
    );
  }
  @Get('analytics') analytics(
    @Param('botId') botId: string,
    @Req() request: Request,
    @Query() query: FinanceUltimateQueryDto,
  ) {
    return this.withUltimate(botId, request, 'DEEP_ANALYTICS', (profileId) =>
      this.ultimate.analytics(profileId, query),
    );
  }
  @Get('items') items(
    @Param('botId') botId: string,
    ...
    return this.withUltimate(botId, request, 'ITEM_ANALYTICS', (profileId) =>
      this.ultimate.items(profileId, query),
    );
  }
  @Get('insights') insights(
    ...
    return this.withUltimate(
      ...
      request,
      'AUTOMATIC_INSIGHTS',
      (profileId) => this.ultimate.insights(profileId),
    );
  }
  @Get('anomalies') anomalies(
    ...
    return this.withUltimate(botId, request, 'ANOMALY_DETECTION', (profileId) =>
      this.ultimate.anomalies(profileId),
    );
  }
  @Post('ask') ask(
    @Param('botId') botId: string,
    ...
    return this.withUltimate(
      ...
      request,
      'FINANCE_HISTORY_QA',
      (profileId) => this.ultimate.answer(profileId, body),
    );
  }

  private async withUltimate<T>(
    botId: string,
    request: Request,
    capability: FinanceCapability,
    action: (profileId: string) => Promise<T>,
  ) {
