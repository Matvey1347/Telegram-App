...
export class StripeBillingProvider {
  ...
  async ensurePrice(input: { botIntegrationId: string; mode: BotBillingProviderMode; plan: { id: string; name: string; description: string | null }; price: { id: string; currency: string; interval: 'MONTH' | 'YEAR'; amountMinor: number; version: number; providerPriceIdentity: Prisma.JsonValue | null } }) {
    ...
    if (current?.priceId) return current.priceId;
    try {
      ...
      const next = { ...identities, STRIPE: { ...((identities.STRIPE as Record<string, unknown>) || {}), [input.mode]: { productId: product.id, priceId: price.id } } };
  ...
  async checkout(input: { botIntegrationId: string; mode: BotBillingProviderMode; customerId: string; priceId: string; subscriptionId: string; successUrl: string; cancelUrl: string; discount?: { code: string; percentOff?: number | null; amountOffMinor?: number | null; currency?: string | null } }) {
    const { key } = await this.resolvedConfig(input.botIntegrationId, input.mode);
    const stripe = new Stripe(key);
    const coupon = input.discount ? await stripe.coupons.create({ duration: 'once', name: input.discount.code, ...(input.discount.percentOff ? { percent_off: input.discount.percentOff } : { amount_off: input.discount.amountOffMinor!, currency: input.discount.currency!.toLowerCase() }) }, { idempotencyKey: `coupon:${input.subscriptionId}` }) : null;
    const session = await stripe.checkout.sessions.create({ mode: 'subscription', customer: input.customerId, line_items: [{ price: input.priceId, quantity: 1 }], ...(coupon ? { discounts: [{ coupon: coupon.id }] } : {}), success_url: input.successUrl, cancel_url: input.cancelUrl, client_reference_id: input.subscriptionId, metadata: { botIntegrationId: input.botIntegrationId, subscriptionId: input.subscriptionId }, subscription_data: { metadata: { botIntegrationId: input.botIntegrationId, subscriptionId: input.subscriptionId } } }, { idempotencyKey: `checkout:${input.subscriptionId}` });
