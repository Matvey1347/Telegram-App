...
describe('FinanceBotChatResponderService', () => {
  ...
  it('returns a useful localized error when the compact recent read fails', async () => {
    ...
    await responder.sendRecentTransactions(
      context,
      'user-1',
      'profile-1',
      'chat-1',
  ...
  it('keeps account actions bounded and two-column', async () => {
    ...
    expect(
      buttons.slice(1, -1).every((row: unknown[]) => row.length <= 2),
    ).toBe(true);
  });

  it('shows compact edit and delete actions beside every category', async () => {
    ...
    await responder.sendCategories(
      context,
      'user-1',
      'profile-1',
      'chat-1',
    ...
    expect(buttons[1]).toEqual([
      expect.objectContaining({
        callbackData: 'fin:flow:edit-category:category-0',
        text: expect.stringMatching(/^✏️ .*Їжа/),
      }),
      {
  ...
  it.each([
    ...
    async (count, missing) => {
      ...
      expect(message.inlineButtons.flat()).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ callbackData: 'fin:flow:start-account' }),
          expect.objectContaining({
            webAppUrl: expect.stringContaining('screen=accounts'),
          }),
        ]),
      );
    },
  ...
  it.each([
    ...
    'renders useful %s settings with plan and full-settings actions',
    async (locale, wording) => {
      ...
      expect(message.inlineButtons.flat()).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({
            webAppUrl: expect.stringContaining('/finance/finance-bot'),
          }),
        ]),
      );
    },
