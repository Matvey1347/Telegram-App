...
describe('TelegramWorkspaceSyncTasksService automatic selections', () => {
  ...
  beforeEach(() => {
    ...
    channels.syncPostsMetricsForWorkspace.mockResolvedValue({
      syncedPosts: 0,
      changedPosts: 0,
      snapshotsCreated: 0,
      affectedDays: 0,
    ...
    prisma.telegramChannelAdminLink.findFirst.mockResolvedValue({
  ...
  it('uses post-metrics and channel-stats flags independently', async () => {
    prisma.telegramChannel.findMany
      ...
      .mockResolvedValueOnce([
        { id: 'stats', autoSyncEnabled: true, syncIncludeChannelStats: true },
      ]);

    await service.syncPostMetricsForWorkspace('workspace-1');
    await service.syncBroadcastStatsForWorkspace('workspace-1');

    expect(channels.syncPostsMetricsForWorkspace).toHaveBeenCalledWith(
      'workspace-1',
      'metrics',
      { postLimit: 100 },
    );
    expect(channels.syncBroadcastStatsForWorkspace).toHaveBeenCalledWith(
      'workspace-1',
      'stats',
      'account-1',
    );
  ...
  it('syncs a published ad placement even when general post metrics are disabled', async () => {
    prisma.telegramChannel.findMany.mockResolvedValueOnce([
      {
        id: 'ad-placement-channel',
        autoSyncEnabled: true,
        syncIncludePostMetrics: false,
        _count: { adSalePlacements: 1 },
      },
      {
        id: 'ordinary-disabled-channel',
        autoSyncEnabled: true,
        syncIncludePostMetrics: false,
        _count: { adSalePlacements: 0 },
      },
    ...
    expect(channels.syncPostsMetricsForWorkspace).toHaveBeenCalledTimes(1);
    expect(channels.syncPostsMetricsForWorkspace).toHaveBeenCalledWith(
      'workspace-1',
      'ad-placement-channel',
      { postLimit: 100 },
    );
    expect(result.summary).toContain('selection skipped 1');
  });
});
