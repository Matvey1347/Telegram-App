...
describe('TelegramPostCalendarPlannerService', () => {
  const setup = () => {
    const prisma = {
      ...
      telegramPostPlannerFormat: {
        count: jest
          .fn()
          .mockImplementation(async (args) => args?.where?.id?.in?.length ?? 1),
  ...
  it('uses each publishing time only once when formats share the same slot', async () => {
    const { prisma, service } = setup();
    const existingSlots = await prisma.telegramPostPlannerSlot.findMany();
    prisma.telegramPostPlannerSlot.findMany.mockResolvedValue([
      ...existingSlots,
      {
        ...existingSlots[0],
        id: 'slot-duplicate',
    ...
    const result = await service.preview('user-1', 'channel-1', {
      from: '2099-08-10',
      to: '2099-08-10',
      formatWeights: { 'format-1': 50, 'format-2': 50 },
    });

    expect(result.summary.availableSlots).toBe(2);
    expect(
      new Set(result.assignments.map((assignment) => assignment.scheduledAt))
        .size,
    ).toBe(result.assignments.length);
  });

  it('excludes posts already used by other days when rerolling a preview', async () => {
    const { service } = setup();
  ...
  it('rerolls a day by rotating eligible posts without changing slots', async () => {
    ...
    expect(
      result.assignments.map((assignment) => assignment.postId),
    ).not.toEqual(baseline.assignments.map((assignment) => assignment.postId));
    expect(result.assignments.map((assignment) => assignment.slotId)).toEqual([
      'slot-1',
      'slot-2',
    ]);
  ...
  it('applies through managed-post batch scheduling and stores provenance', async () => {
    ...
    expect(
      telegramChannelsService.scheduleManagedPostsBatch,
