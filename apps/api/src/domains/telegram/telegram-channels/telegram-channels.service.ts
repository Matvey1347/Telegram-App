...
import { TELEGRAM_SYSTEM_BOT_SOURCE_ID } from '../../../telegram/shared/telegram-source-access.service';
...
export class TelegramChannelsService {
  ...
  constructor(
    ...
    private readonly identityService: TelegramManagedPostIdentityService = new TelegramManagedPostIdentityService(
      prisma,
    ),
    private readonly botApiClient: TelegramBotApiClient = new TelegramBotApiClient(),
  ...
  private async refreshSystemBotPublishingAccess(
    workspaceId: string,
    channel: {
      id: string;
  ...
  private async botTokenForSource(workspaceId: string, sourceId: string) {
  ...
  private async resolveImportPolicy(params: {
    ...
    const inviteLinksSyncFromInput = this.toOptionalDate(
  ...
  private async refreshMtprotoAccountCapabilities(account: {
    id: string;
    label: string;
    apiId: string;
    apiHashEncrypted: string;
  ...
  private async persistInviteLinkFromRemote(params: {
    ...
    maps: Awaited<
      ReturnType<TelegramChannelsService['buildInviteAttributionMaps']>
    >;
  ...
  private buildInviteLinkHistoryPayload(
    inviteLink: {
      joinedCount: number;
    ...
    const historyRows = this.appendCurrentInviteLinkHistoryRowIfChanged(
      rowsAsc.slice(-maxPoints),
      {
        syncedAt:
  ...
  private async preloadCampaignInviteLinkHistories(
    ...
    if (!rows.length) {
      return new Map<
        string,
        ReturnType<
          TelegramChannelsService['buildCampaignInviteLinkHistoryPayload']
        >
      >();
    }
  ...
  >(workspaceId: string, channelId: string, links: T[], limit = 120) {
    const normalizedLinks = this.normalizeInviteLinksRequestedCount(links);
  ...
  private async editManagedPostTextInTelegram(params: {
    ...
    if (source.sourceType === TelegramSourceType.MTPROTO) {
      if (rendered.richHtml) {
        ...
        const token = await this.botTokenForSource(
          workspaceId,
          botSource.sourceId,
        );
        const chatId = this.botChatId(channel);
        ...
        const published = await this.botApiClient.call<{ message_id: number }>(
          token,
          'sendRichMessage',
          {
            chat_id: chatId,
    ...
    const token = await this.botTokenForSource(workspaceId, source.sourceId);
    const chatId = this.botChatId(channel);
    if (!chatId) {
      throw new BadRequestException('Channel has no Telegram chat id');
    }
    ...
    const call = async (method: string, body: Record<string, unknown>) => {
      try {
        ...
        if (this.isBotMessageNotModified(description)) {
          unchangedCount += 1;
          return false;
        }
        throw error;
  ...
  private async normalizePostGroupNumbering(
  ...
  private async normalizeChannelPostGroupNumberingOnRead(
    workspaceId: string,
    channelId: string,
  ) {
  ...
  async postGroups(userId: string, query: PostGroupsQueryDto) {
    ...
    if (query.telegramChannelId) {
      ...
      await this.normalizeChannelPostGroupNumberingOnRead(
        workspaceId,
        query.telegramChannelId,
      );
    }
  ...
  async postGroup(userId: string, groupId: string) {
    ...
    if (!group) throw new NotFoundException('Post group not found');
    await this.prisma.$transaction(async (tx) => {
      await this.normalizePostGroupNumbering(tx, groupId);
    });
  ...
  async createPostGroup(userId: string, dto: CreatePostGroupDto) {
    ...
    const previousGroupIds = await this.prisma.$transaction(async (tx) => {
      ...
      for (const oldGroupId of oldGroupIds) {
        await this.normalizePostGroupNumbering(tx, oldGroupId);
      }
  ...
  async managedPosts(userId: string, channelId: string) {
    ...
    try {
      const reconciliation = await this.reconcileDueManagedPosts(
        workspaceId,
        channelId,
      );
  ...
  private async publishDueLocallyScheduledManagedPosts() {
  ...
  private async reconcileManagedPostIdentities(params: {
    workspaceId: string;
    channelId?: string;
    postId?: string;
    explicit?: boolean;
    ...
    return this.identityService.reconcile({
      ...
      loadRemote: async (channelId, posts) => {
        ...
        const channelReference = this.mtprotoChannelReference(
  ...
  async verifyManagedPostTelegramId(
    ...
    const [hydrated] = await this.attachManagedPostIcons([updated]);
    return hydrated;
  }
  ...
  private async repairScheduledPostDependants(
    workspaceId: string,
    channelId: string,
    targetPostId: string,
    publishedAt: Date,
      ...
      if (!messages.length) {
        if (post.status === 'SCHEDULED') {
          ...
          await this.prisma.$transaction(async (tx) => {
            ...
            if (post.groupId) {
              await this.normalizePostGroupNumbering(tx, post.groupId);
            }
          });
          await onProgress?.(
            {
  ...
  private async syncRemoteScheduledManagedPosts(params: {
        ...
        if (shouldReviveImportedPost) {
          ...
          const revived = await this.prisma.$transaction(async (tx) => {
            ...
            if (previousGroupId && previousGroupId !== importedSystemGroup.id) {
              await this.normalizePostGroupNumbering(tx, previousGroupId);
            }
            return updated;
          });
    ...
    if (importedSystemGroupTouched) {
      await this.prisma.$transaction((tx) =>
        this.normalizePostGroupNumbering(tx, importedSystemGroup.id),
      );
    }
  ...
  async setManagedPostTelegramUrl(
    ...
    if (!normalizedInput) {
      const publishedPost = await this.prisma.$transaction(async (tx) => {
        ...
        if (currentPost.groupId) {
          await this.normalizePostGroupNumbering(tx, currentPost.groupId);
        }
      ...
      const [hydrated] = await this.attachManagedPostIcons([publishedPost]);
      return hydrated;
    }
    ...
    const [hydrated] = await this.attachManagedPostIcons([linkedPost]);
    return hydrated;
  }
  ...
  private resolveInternalPostLinksForPublish(
    workspaceId: string,
  ...
  async createManagedPost(
    ...
    const [hydrated] = await this.attachManagedPostIcons([post]);
    return hydrated;
  }
  ...
  async managedPostHistory(userId: string, channelId: string, postId: string) {
    ...
    return this.attachManagedPostIcons(
      await this.listManagedPostRevisions(this.prisma, {
        telegramManagedPostId: postId,
        workspaceId,
      }),
    );
  ...
  async restoreManagedPostRevision(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const affectedGroupId of affectedGroupIds) {
        await this.normalizePostGroupNumbering(tx, affectedGroupId);
      }
      const canonical = await tx.telegramManagedPost.findUnique({
  ...
  async updateManagedPost(
    ...
    if (convertsNativeScheduleToLocal) {
      ...
      await this.cancelScheduledManagedPost(workspaceId, {
        ...post,
    ...
    const [hydrated] = await this.attachManagedPostIcons([updatedPost]);
    return hydrated;
  }
  ...
  private async publishManagedPost(
    ...
    try {
      ...
      if (source.sourceType === TelegramSourceType.MTPROTO) {
        ...
        if (post.imageUrls.length > 1) {
          ...
          if (richHtml && textHtmlParts.length === 1) {
            const result = (await call('sendRichMessage', {
      ...
      const publishedUrls = scheduleAt
        ? []
        : this.telegramMessageUrlsForPost(channel, ids, post.imageUrls.length);
  ...
  async returnManagedPostToDraft(
    ...
    const [hydrated] = await this.attachManagedPostIcons([draftPost]);
    return hydrated;
  }
  ...
  async importChannel(
    ...
    const importPolicy = await this.resolveImportPolicy({
      ...
      existing: existing
        ? {
            acquisitionType:
              (existing as { acquisitionType?: 'CREATED' | 'PURCHASED' | null })
  ...
  async syncNow(
    ...
    const postLimit =
      dto.postLimit || (await this.postSyncLimitForChannel(channelId));
    ...
    if (selection.syncIncludeOlderPosts) {
      ...
      olderPostsBackfill = await this.syncOlderPostsMetricsBackfillForWorkspace(
        workspaceId,
        channelId,
        {
          telegramUserAccountId: account.id,
    ...
    if (selection.syncIncludeAudienceSnapshot) {
      ...
      audienceSnapshot = await this.createAudienceSnapshotSafely(
        channelId,
        'sync',
      );
      steps.push(
  ...
  async syncHistorical(
    ...
    let inviteResult: Awaited<
      ReturnType<TelegramChannelsService['syncChannelInviteLinks']>
    > | null = null;
  ...
  async syncPostsMetricsForWorkspace(
    ...
    if (!channel) throw new NotFoundException('Telegram channel not found');
    const importPolicy = await this.getChannelSyncCutoffs(
      workspaceId,
      channelId,
    );
    const account = await this.connectedAccount(
  ...
  async inviteLinkHistory(
    ...
    return this.buildInviteLinkHistoryPayload(
      inviteLink,
