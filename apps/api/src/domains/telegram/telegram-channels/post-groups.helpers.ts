...
export function movedPostDatabaseState(
  ...
  return {
    ...
    telegramIdVerificationStatus: 'UNVERIFIED' as const,
...
type ManagedPostNumberingClient = Pick<
  Prisma.TransactionClient,
  'telegramManagedPost' | '$executeRaw'
>;

export async function normalizePostGroupsNumbering(
  tx: ManagedPostNumberingClient,
  groupIds: string[],
) {
  ...
  const statusPositions = new Map<
    string,
    Record<ManagedPostNumberingBucket, number>
  >();
  const rows = posts.map((post) => {
    const groupId = post.groupId as string;
    const groupPosition = groupPositions.get(groupId) ?? 0;
    groupPositions.set(groupId, groupPosition + 1);
    const counters = statusPositions.get(groupId) ?? {
      DRAFT: 0,
      SCHEDULED: 0,
      PUBLISHED: 0,
    };
    statusPositions.set(groupId, counters);
  ...
  await tx.$executeRaw`
    ...
    WHERE post."id" = data."id"
      AND (post."groupPosition", post."statusPosition") IS DISTINCT FROM
          (data."groupPosition"::integer, data."statusPosition"::integer)
  `;
}
...
export async function normalizePostGroupNumbering(
  ...
  await normalizePostGroupsNumbering(tx, [groupId]);
}
