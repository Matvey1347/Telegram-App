...
describe('post group helpers', () => {
  ...
  it('normalizes multiple groups in one read and one conditional update', async () => {
    const tx = {
      telegramManagedPost: {
    ...
    await normalizePostGroupsNumbering(tx as never, ['group-a', 'group-b']);

    expect(tx.telegramManagedPost.findMany).toHaveBeenCalledTimes(1);
    expect(tx.$executeRaw).toHaveBeenCalledTimes(1);
  });
