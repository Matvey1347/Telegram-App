...
- Business modules belong under `src/domains/<group>/<module>`. Use the existing groups `finance`, `growth`, `identity`, `operations`, `telegram`, and `workspace`; do not add another top-level business folder.
...
- For Finance Bot or Finance Mini App consumer API work, read `docs/finance-bot/PRODUCT_RULES.md` before implementation.
...
- Read `docs/runtime-cost-efficiency.md` for any production behavior. A cron, interval, timeout loop, DB wakeup, external API loop, snapshot, or persisted log needs a cost review.
- Do not add one-minute polling without explicit technical justification. Prefer a persisted `nextDueAt`/`nextScheduledRunAt` and one due timer; empty runs should perform no DB write and ideally no DB query.
- Materialize task configuration at bootstrap, workspace creation, or lazy workspace access, never through a frequent all-workspace scan. Preserve leases, claims, idempotency and restart recovery.
- Batch bounded reads/writes, query only enabled scopes, avoid unchanged-state upserts, and summarize job results rather than logging each item.
- Application INFO/debug telemetry and successful request completions are not PostgreSQL records by default. Keep audit/security/errors, and give append-only data retention, indexes and downsampling.
...
- `rg "describe\\(|it\\(" apps/api/src/domains/<group>/<module>`
