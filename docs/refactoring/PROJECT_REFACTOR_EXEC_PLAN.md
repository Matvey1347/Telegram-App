...
ledger: eligible Telegram income less confirmed operating expenses and the
seller commission. Each event is allocated using capital accumulated strictly
before that event, so a later contribution cannot alter an earlier split. The
calculation is read-time and uses bounded workspace reads, adding no timer,
polling, background work, duplicated ledger rows, or per-member HTTP joins.
Manual reinvestment distribution and withdrawal controls are no longer exposed;
salary payout and salary-to-capital remain separate decisions.

## 2026-09-20 System-wide exchange-rate reference data

Exchange-rate observations are now system-owned rather than copied into every
workspace. The database keeps one historical observation per
base-currency/target-currency/UTC-day; the migration deterministically merges
legacy duplicates, preferring official-provider data. Workspace currency
settings remain workspace-local because they only choose how that workspace
displays amounts.

The manual create, edit, delete, and sync rate surfaces were removed from both
the API and the Finance UI. A non-editable system-maintenance task fetches the
canonical EUR rate graph once daily. Historical and lazy current-pair lookups
read the same global graph, with de-duplicated writes and no workspace scan.
At 100 workspaces this remains one scheduled provider request and one bounded
batch insert per day, rather than 100 independent syncs and copies.

## 2026-09-17 Autogenerated CPM pricing and canonical sorting controls

Autogenerated channel messages can now render either the existing public sales
price or a price calculated from each channel's internal CPM. Both variants are
prepared from the same bounded channel/product pricing batch; changing the
editor option performs no additional request and does not duplicate financial
math in the browser. A missing internal CPM is shown as unavailable instead of
silently falling back to the public price. The existing short channel
description remains sourced from Appearance settings and its API limit now
matches the 240-character editor limit.

Internal paginated lists use a canonical field selector plus an adjacent
ascending/descending button. Ad campaigns and CRM clients share the internal
control; Consumer Finance keeps a product-owned localized equivalent. Campaign
cost and joined ordering and investment ordering execute in the database before
pagination with deterministic id tie-breakers. Investment monetary sorting uses
valuation-currency-normalized columns so unlike currencies are not compared as
if they were equal. The template calculation reuses one pricing read for up to
100 channels, and sorting adds no timer, polling, background process, unchanged
write, or per-row HTTP/SQL request.

## 2026-09-17 Telegram channel traffic-source attribution

Telegram channel cards now expose a compact source-attribution summary and a
lazy-loaded detail modal for mutual promotion, folders, ad campaigns, audience
transfer, bot, broadcast, and otherwise unclassified invite links. The stable
shared read model reports acquired peak, retained audience, drop from peak,
paid-source CPA, per-source rows, and a bounded history series. Folder rows use
their saved attribution boundaries and primary-currency expense; ad campaigns
use their primary-currency campaign spend. One invite link is classified under
one source kind so overlapping campaign/folder/mutual configuration cannot
double-count it, while separate reuse windows inside the same source kind stay
visible as separate rows.

The channel list remains one HTTP request and performs six batched,
workspace-scoped reads for up to 100 channel cards, plus one bounded lateral
boundary lookup when completed mutual placements are present. It does not scan
raw invite history to calculate peaks: `TelegramInviteLink.peakAttributedCount`
is backfilled from current counters and existing snapshots, then advanced in
the existing invite sync upsert and protected from concurrent regression by a
database trigger. Direct-mutual JSON targets are filtered in PostgreSQL and
covered by a GIN index; the campaign lookup has a
workspace/channel/analytics index.
The detail endpoint is fetched only when the user opens the modal and caps raw
history at 5,000 rows. No timer, polling loop, background process, external
request, unchanged-state write, or new retention stream is introduced.

Frontend list/detail cache keys are separate and sync completion invalidates
only the affected attribution family. The legacy channels page shrank by
extracting its cohesive catalog card, and its shrinking-only size/API-import
allowances were lowered in the same change. Focused backend and frontend tests
cover calculations, workspace isolation, cross-source deduplication,
same-source folder reuse, the 100-channel bounded-read case, compact card UI,
loading/error/retry/empty/detail modal states, and sync invalidation.

## 2026-09-15 Canonical workspace-local modal draft sessions

This completed frontend refactor makes the existing workspace modal draft storage,
React hook, and picker the only lifecycle for unfinished browser-local forms.
Features continue to own their serializable value, seeded defaults,
normalization, meaningfulness rule, restore mapping, and compact preview data.
The shared layer owns workspace keys, IDs and timestamps, legacy decoding,
initialization, autosave deduplication, multiple drafts, picker transitions,
and successful-submit cleanup.

The accepted runtime boundary is localStorage only. It adds no API, React Query,
PostgreSQL/Neon read or write, Railway process, background job, service worker,
interval, external lookup, or query invalidation. New picker metadata is stored
with each draft and rendered locally; legacy rows degrade to a local fallback
instead of fetching icons. Selecting 100 channels changes only the browser-local
serialized form and performs zero backend operations.

Grace Explorer mapped the shared and bespoke draft paths, Knuth Performance
confirmed the zero-backend cost boundary, and Parnas Structure completed both
the read-only preflight and clean final structural review. Ada Frontend migrated
Ad Sale, Promo, both Cross Promotion modes, Mutual Folder create/edit, Post
Batch, publication schedules, Campaign/Hypothesis, and message templates.
Turing Review found and drove fixes for a workspace-switch autosave race and
untouched prop-seeded Ad Sale drafts, then completed a clean final re-review.

The final storage contract uses a versioned multi-draft envelope with immutable
creation time, change-only update time, local preview metadata, normalization,
corrupt-data repair, quota-safe writes, and merged canonical/legacy-key
migration. A generation guard prevents a previous workspace or modal invocation
from writing through a newly initialized session. Successful-submit cleanup
invalidates the in-memory envelope, so a later render cannot recreate it.

Final focused validation passes 16 Web files/74 tests, including storage/hook,
picker, Ad Sale characterization, Promo, both Cross modes, Mutual Folder,
Post Batch, schedules-related editor behavior, and message templates. Web
typecheck and production build, targeted ESLint, architecture policy 18/18,
and `git diff --check` pass. The normal architecture size stage retains seven
unrelated surrounding-worktree shrinking-baseline overruns; this slice removed
the prior Ad Campaigns page overrun, lowered its allowance from 2464 to 2429,
and kept every touched draft production unit at or below 400 lines. The new
developer guide is `docs/design-system/MODAL_DRAFTS.md`.

Runtime acceptance remains exact: additional Neon reads and writes are zero;
Railway background CPU and recurring jobs are zero; normal autosave performs
zero external HTTP requests; selecting 100 channels performs zero backend
operations. Storage grows only with meaningful local drafts created by the
user. Writes occur only after a changed meaningful browser snapshot; identical
state is deduplicated and empty seeds are removed. No backend service, database
schema, React Query persistence, service worker, timer, or background sync was
added for modal drafts.

## 2026-09-15 Canonical website-driven System Bot post import

This completed refactor replaces feature-specific Ad Sale, Promo, Mutual
Promotion, and Post Batch capture transports with one Telegram-owned contract.
The public capture modes are only `single` and `multiple`; both return normalized
`TelegramSystemBotPostDraft[]`. Business consumers remain responsible for
channels, slots, scheduling, validation, and persistence after capture. The
ordinary `/post` bot wizard retains its separate channel/publication semantics.

The accepted runtime design is request/event-driven. Idle, hidden, terminal,
and unmounted consumers perform no import reads. One visible active workflow
uses one bounded, visibility-aware reconciler with a slower backoff than the
legacy feature hooks, and active reads use a compact workflow projection rather
than repeatedly loading the growing JSON payload. No cron, worker, database
heartbeat, SSE/WebSocket, persistent cache, telemetry writer, service, or
unchanged-state write is introduced. Capture cost is independent of selected
channel count; channel fan-out begins only in the owning domain submit flow.

Grace Explorer and Knuth Performance mapped the original paths and costs;
Parnas Structure completed preflight and a clean post-remediation audit. The
initial worktree was clean. All changed handwritten services, hooks, and React
components are below their applicable size targets, the canonical layers have
no reverse Growth dependency, and old feature-specific runtime paths are gone.
Architecture policy tests pass 17/17; the normal gate retains only the same
eight unrelated pre-existing shrinking-baseline overruns. Turing Review found
and drove fixes for OWN_CHANNELS access, recovery ownership and target routing,
single-poller handoff, same-workspace cross-user consumption, and same-mode
start hijacking before its final integration pass.

Final focused validation passes 8 API suites/80 tests and 8 Web files/45
tests. Shared typecheck/build, API build, Web typecheck/build, Prisma client
generation and schema validation, targeted API/Web lint, and diff checks pass.
API typecheck remains red only on the pre-existing
`telegram-channel-catalog-select.spec.ts` preview typing error. The full Web
suite passes 1,285/1,298 tests; its 13 failures are in nine unrelated legacy
suites. The full API run exposes the existing Regular Payments mock failure and
development force-exit behavior; all refactor-focused suites pass.

Runtime cost remains bounded and request-driven. Idle imports add zero queries,
writes, timers, processes, or connections. A visible ACTIVE import has one
module-coordinated poller: one compact metadata row per read, every second for
the first ten seconds and every three seconds afterward, capped at 120 visible
seconds (about 47 reads at the cap versus about 94 in the replaced flow).
Hidden, unmounted, timed-out, and terminal states read nothing. Start writes one
workflow and attaches one control message; each accepted forward performs one
versioned payload transition, albums update the same logical draft, and
finish/cancel performs one versioned status transition. No unchanged-state
writes were added. Completed payload is loaded once, and downstream consumers
claim it atomically with their domain write.

Telegram traffic occurs only for user actions: create/edit the control card,
existing media download/storage calls for forwarded media, input cleanup, and
one or two preview messages depending on content. Capture is O(1) in selected
channel count; 100-channel fan-out remains solely in the later domain publish
operation. No Railway service, worker, Redis, heartbeat, SSE/WebSocket, or
long-lived connection was added, so idle CPU/RAM/network and Neon scale-to-zero
behavior are unchanged. Persistence adds one workflow row per import under the
existing workflow retention policy; no new retention stream or cleanup job was
introduced.
